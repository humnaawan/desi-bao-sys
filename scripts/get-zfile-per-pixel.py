...
parser.add_option('--coadds-path',
                  ...
                  help='path to directory that contains the spectra-<nside> folder\
                        (i.e., where subfolders are  healpix pixel number)')
...
for hpix in os.listdir(f'{coadds_path}/spectra-{nside}/'):
    logging.info(f' ## working with healpix pixel= {hpix}')
    # read in the coadded spectra
    fname = f'{coadds_path}/spectra-{nside}/{hpix}/spectra-{nside}-{hpix}.fits'
    ...
    for night in np.unique(fibermap_exp['NIGHT']):
        ...
        for expid in np.unique(fibermap_exp['EXPID']):
            ...
            if len(inds_to_pull) != len(np.unique(targetids)):
                raise ValueError(f' ## somethings wrong: n-exp ({len(inds_to_pull)}) != unique targets ({len(np.unique(targetids))})')
    ...
    # write this data out
    fname = f'{coadds_path}/spectra-{nside}/{hpix}/ztrue.fits'
